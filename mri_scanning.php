...
//my booking list
$booked = "SELECT * FROM `appointment_mri` WHERE `Username` = '{$_SESSION['username']}'";
...
if (isset($_POST['showTime'])) {
    ...
    $appoinment = "SELECT * FROM `appointment_mri` WHERE `Hospital` = '{$Hospital}' AND `Date` = '{$Date}' AND (`Status` = 1 OR `Status` = 2)";
    $appoinment_query = mysqli_query($db, $appoinment);
} else {
    $appoinment = "SELECT * FROM `appointment_mri`";
    $appoinment_query = mysqli_query($db, $appoinment);
}
                ...
                <div class="row align-items-center">
                    <div class="col-sm-6">
                        <div class="breadcrumbs-area clearfix">
                            <h4 class="page-title pull-left">MRI Scann</h4>
                            <ul class="breadcrumbs pull-left">
                                <li><a href="index.php">Home</a></li>
                                <li><span>MRI Scanning Machine</span></li>
                            </ul>
                        </div>
                    </div>
                ...
                <div class="NetHub_form_aling">
                    ...
                    if (mysqli_num_rows($booked_query) > 0) {
                        ...
                        while ($fetchList = mysqli_fetch_assoc($booked_query)) {
                            ...
                            if ($fetchStatus == 1) {
                                $link = "<a href='mri_scanning.php?cancel={$fetchID}'>Cancel</a>";
